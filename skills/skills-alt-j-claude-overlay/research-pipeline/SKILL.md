...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, WebFetch, Agent, Skill, mcp__gemini-review__review, mcp__gemini-review__review_reply, mcp__gemini-review__review_start, mcp__gemini-review__review_reply_start, mcp__gemini-review__review_status
---
...
- **REVIEWER_DIFFICULTY = medium** — How adversarial the reviewer is. `medium` (default): standard MCP review (Gemini via `gemini-review`). `hard`: adds reviewer memory + debate protocol. `nightmare`: NOT SUPPORTED on Alt J (would require Codex `exec`); falls back to `hard`. Passed through to `/auto-review-loop`.
...
1. Gemini (via `gemini-review` MCP) reviews the work (score, weaknesses, minimum fixes)
