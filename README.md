...
| **Alt J** 🆕 | Claude Code CLI (Vertex) | Gemini CLI (`gemini-review` MCP) | No* | No* | [OPENCLAW_ORCHESTRATOR_GUIDE](docs/OPENCLAW_ORCHESTRATOR_GUIDE.md) |
...
**Alt J** uses [OpenClaw](https://docs.openclaw.ai) as a top-level orchestrator that drives stage transitions across the ARIS workflow. The actual heavy work (code, experiments, paper writing) is delegated to **Claude Code CLI backed by Google Vertex AI** as the executor, while **Gemini CLI** serves as the cross-family reviewer through the existing `gemini-review` MCP bridge. This gives you a single conversational entry point (OpenClaw chat) that orchestrates a Vertex-driven Claude executor and a Gemini reviewer — useful when you want long-running unattended research with a chat-style supervisor and zero direct OpenAI/Anthropic billing (Vertex covers Claude billing via GCP).

\* Alt G normally relies on local Codex CLI and Claude Code CLI logins. Direct API keys are optional, not required. Alt J relies on a configured Google Vertex AI project for Claude billing and a Gemini CLI login (or `GEMINI_API_KEY`); no direct Anthropic or OpenAI keys are required.
